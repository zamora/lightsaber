...
; Contract for the supported hilt styles
(define hilt-style? (flat-contract (one-of/c 'luke 'vader 'kylo 'maul)))
...
(define/contract (lightsaber color
  ...
  (let* ([the-hilt (hilt hilt-style)]
         [the-blade (blade color-obj length)])
    (cond
      ...
      ; disadvantages.
      [(eq? hilt-style 'kylo) (let ([cross (blade color-obj (/ length 2.75))])
                                (lc-superimpose (inset (rotate cross (/ pi 2))
                                                ...
                                                (inset the-blade
                                                       ...
                                                       0 0)
                                                the-hilt))]
      [else (lc-superimpose (inset the-blade
                                   (hilt-horiz-offset hilt-style)
                                   (hilt-vert-offset hilt-style)
                                   0 0)
                            the-hilt)])))
...
; How much to offset the blade for the specific style of hilt
...
(define kylo-hilt
  (lc-superimpose
   ...
   (let ([quillon (lt-superimpose (inset (shear (filled-rectangle 13 10
     ...
     (inset (vc-append 46 quillon (flip-vertical quillon))
            114 0 0 0))
...
;-------------------------------------------------------
;
; Helper function to draw shaded gradient for Darth Maul's hilt
(define (maul-segment width)
  (cc-superimpose (filled-rectangle (+ width 2) 36 #:color "Black" #:border-width 2)
...
(define maul-hilt
  (lt-superimpose
   (hc-append
    ...
    ; Shear to a 20 degree angle, then rotate 45 degrees, then clip
    (let* ([size 64]
           [diamond (rotate (shear (filled-rectangle size size #:color (gray 210) #:border-width 2)
                                   1.375 1.375)
                            0.7854)]
           [width (pict-width diamond)]
           [height (pict-height diamond)]
           [offset (round (/ (- height size) -2))])
      (inset/clip diamond (/ width -2) offset (/ width -3) offset))
   ...
   (inset (rotate (linewidth 2 (hline 29 4)) -0.0875) 97 21.5 0 0)
   (inset (linewidth 2 (hline 29 2)) 97 31.5 0 0)
   (inset (rotate (linewidth 2 (hline 29 4)) 0.0875) 97 37 0 0)
...
(lightsaber "OrangeRed" #:style 'kylo)
(lightsaber "Cyan" #:style 'maul)
(define single (lightsaber "ForestGreen" #:length 200 #:style 'maul))
